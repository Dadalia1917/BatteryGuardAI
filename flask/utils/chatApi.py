...
class ChatAPI:
    ...
    def local_gemma_request(self, messages):
        """使用本地部署的Gemma3模型 - 通过LM-studio"""
        try:
            print(f"正在向本地Gemma3模型发送请求...")
            ...
            if response.status_code != 200:
                ...
                return "Gemma3离线不可用，请检查本地LM-studio服务是否正常运行"
                
            result = response.json()
            print(f"收到Gemma3模型响应: {str(result)[:100]}...")
            
            if 'choices' in result and len(result['choices']) > 0 and 'message' in result['choices'][0]:
                return result['choices'][0]['message']['content']
            else:
                print(f"Gemma3响应格式错误: {result}")
                return "Gemma3响应格式错误，请检查LM-studio模型输出"
        except requests.Timeout:
            print("Gemma3请求超时")
            return "Gemma3请求超时，请检查本地LM-studio服务是否正常运行"
        except requests.ConnectionError:
    ...
    def lan_gemma_request(self, messages):
        """使用局域网部署的Gemma3模型 - 通过LM-studio"""
        try:
            print(f"正在向局域网Gemma3模型发送请求...")
            ...
            if response.status_code != 200:
                ...
                return "Gemma3离线不可用，请检查局域网LM-studio服务是否正常运行"
                
            result = response.json()
            print(f"收到局域网Gemma3模型响应: {str(result)[:100]}...")
            ...
            print("局域网Gemma3请求超时")
            return "Gemma3请求超时，请检查局域网LM-studio服务是否正常运行"
        except requests.ConnectionError:
            print("无法连接到局域网LM-studio服务")
            return "Gemma3离线不可用，请检查局域网LM-studio服务是否正常运行"
        except Exception as e:
    ...
    def local_qwen3_request(self, messages, think_mode=False):
        """使用本地部署的qwen3.0模型 - 通过LM-studio"""
        try:
            # 调试信息
            print(f"接收到的think_mode参数: {think_mode}, 类型: {type(think_mode)}")
            
            # 处理思考模式
            if think_mode:
                messages.append({"role": "system", "content": "请使用思考模式回答问题，先分析问题，然后给出答案。"})
                print("使用思考模式")
            ...
            if response.status_code != 200:
                ...
                return "qwen3.0离线不可用，请检查本地LM-studio服务是否正常运行"
            ...
            if 'choices' in result and len(result['choices']) > 0 and 'message' in result['choices'][0]:
                ...
                return "qwen3.0响应格式错误，请检查LM-studio模型输出"
        except requests.Timeout:
            print("qwen3.0请求超时")
            return "qwen3.0请求超时，请检查本地LM-studio服务是否正常运行"
        except requests.ConnectionError:
            print("无法连接到LM-studio服务")
            return "qwen3.0离线不可用，请检查本地LM-studio服务是否正常运行"
        except Exception as e:
    ...
    def lan_qwen3_request(self, messages, think_mode=False):
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                return "qwen3.0离线不可用，请检查局域网LM-studio服务是否正常运行"
            ...
            return "qwen3.0请求超时，请检查局域网LM-studio服务是否正常运行"
        except requests.ConnectionError:
            print("无法连接到局域网LM-studio服务")
            return "qwen3.0离线不可用，请检查局域网LM-studio服务是否正常运行"
        except Exception as e:
    ...
    def local_qwen25vl_request(self, messages):
        """使用本地部署的qwen2.5-VL模型 - 通过LM-studio"""
        try:
            ...
            if response.status_code != 200:
                ...
                return "qwen2.5-VL离线不可用，请检查本地LM-studio服务是否正常运行"
            ...
            if 'choices' in result and len(result['choices']) > 0 and 'message' in result['choices'][0]:
                ...
                return "qwen2.5-VL响应格式错误，请检查LM-studio模型输出"
        except requests.Timeout:
            print("qwen2.5-VL请求超时")
            return "qwen2.5-VL请求超时，请检查本地LM-studio服务是否正常运行"
        except requests.ConnectionError:
            print("无法连接到LM-studio服务")
            return "qwen2.5-VL离线不可用，请检查本地LM-studio服务是否正常运行"
        except Exception as e:
    ...
    def lan_qwen25vl_request(self, messages):
        """使用局域网部署的qwen2.5-VL模型 - 通过LM-studio"""
        try:
            ...
            if response.status_code != 200:
                ...
                return "qwen2.5-VL离线不可用，请检查局域网LM-studio服务是否正常运行"
            ...
            return "qwen2.5-VL请求超时，请检查局域网LM-studio服务是否正常运行"
        except requests.ConnectionError:
            print("无法连接到局域网LM-studio服务")
            return "qwen2.5-VL离线不可用，请检查局域网LM-studio服务是否正常运行"
        except Exception as e:
