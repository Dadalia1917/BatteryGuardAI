<template>
	<div class="login-container">
		...
		<div class="login-box animate__animated animate__fadeIn">
			<div class="title">
				<h2>基于动态深度学习和大模型的电池故障检测系统</h2>
				<p>Battery fault detection system based on deep learning and large modelsk</p>
			</div>
		...
		if (valid) {
			request.post('/api/user/login', ruleForm).then((res) => {
				...
				if (res.code == 0) {
					...
					Cookies.set('username', res.data.username); // 设置用户名
					localStorage.setItem('username', res.data.username); // 同时在localStorage中也保存用户名
					//登录成功
					onSignIn();
				} else {
					ElMessage({
